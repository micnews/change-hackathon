...
module.exports = function (opts, callback) {
  console.log(opts.url.split('/').pop().split('?')[0]);
  var url = opts.url
  ...
  function finished (err, response, body) {
    ...
    if (!opts.url && (!body.items || !body.items.length)) return callback('Nothing found');
    ...
    const petitionData = {
      ...
      image: (function () {
        if (petition.media) {
          return petition.media.sizes.small.url;
        } else if (petition.photo) {
          return petition.photo.sizes.small.url;
        } else {
          return '//images.mic.com/cmtrubzehmstehoggvxqcvaxgaiqwmeabjw2mrllksjll71omw19kxyeepsw8xgf.jpg';
        }
      })()
    };
